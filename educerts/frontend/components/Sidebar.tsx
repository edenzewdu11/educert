...
import { Shield, LayoutDashboard, FilePlus, Search, Settings, LogOut, User, GraduationCap, Smartphone, Award } from "lucide-react"
  ...
  return (
    <div className="w-64 h-screen bg-white border-r border-slate-200 flex flex-col fixed left-0 top-0 z-50 shadow-sm">
      <div className="p-6 flex items-center gap-3">
        ...
        <div>
          <h1 className="font-bold text-lg tracking-tight text-slate-900">EduCerts<span className="text-sky-600">.io</span></h1>
        ...
        {user?.is_admin ? (
          ...
          navItems.map((item) => {
            ...
            return (
              ...
              >
                <item.icon className={`w-5 h-5 ${isActive ? "text-sky-600" : "text-slate-400 group-hover:text-sky-500"}`} />
        ...
        <div className="bg-slate-50 rounded-2xl p-4 border border-slate-200">
          <div className="flex items-center gap-3 mb-4">
            <div className="w-10 h-10 rounded-full bg-white flex items-center justify-center overflow-hidden border border-slate-200 shadow-sm">
              {user ? <User className="w-5 h-5 text-sky-600" /> : <div className="w-full h-full bg-sky-600" />}
            </div>
          ...
          {user ? (
            <div className="space-y-2">
              {!user.is_admin && (
                <Link
                  href="/wallet"
                  className="w-full flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl bg-sky-600 hover:bg-sky-700 transition-all text-[10px] font-black text-white uppercase tracking-widest shadow-lg shadow-sky-600/20"
                >
            ...
            <Link href="/login" className="w-full flex items-center justify-center gap-2 px-4 py-2 rounded-xl bg-sky-600 hover:bg-sky-700 transition-all text-xs font-semibold text-white">
              Login to EduCerts
            </Link>
          )}
        </div>
