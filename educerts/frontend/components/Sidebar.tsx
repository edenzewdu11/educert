...
export default function Sidebar({ user, onLogout }: { user: any, onLogout: () => void }) {
  ...
  return (
    <>
      ...
      <AnimatePresence>
        {mobileOpen && (
          <motion.div
            ...
            className="lg:hidden fixed left-0 top-0 z-50 h-screen w-64 bg-white border-r border-slate-200 flex flex-col shadow-sm"
          >
      ...
      <div className="hidden lg:flex lg:flex-col lg:fixed lg:left-0 lg:top-0 lg:z-auto lg:h-screen lg:bg-white lg:border-r lg:border-slate-200 lg:shadow-sm lg:w-64 xl:w-72 2xl:w-80">
        <div className="p-4 sm:p-6 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="w-8 h-8 sm:w-10 sm:h-10 bg-sky-600 rounded-xl flex items-center justify-center shadow-lg shadow-sky-500/20">
              <Shield className="w-4 h-4 sm:w-6 sm:h-6 text-white" />
        ...
        <div className="p-3 sm:p-4 mt-auto">
          <div className="bg-slate-50 rounded-2xl p-3 sm:p-4 border border-slate-200">
            ...
            {user ? (
              ...
              <Link href="/login" className="w-full flex items-center justify-center gap-1.5 sm:gap-2 px-3 sm:px-4 py-2 rounded-xl bg-sky-600 hover:bg-sky-700 transition-all text-[10px] sm:text-xs font-semibold text-white">
                Login to EduCerts
              </Link>
            )}
          </div>
