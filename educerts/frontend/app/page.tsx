      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 relative z-10">
        <div>
          <div className="flex items-center gap-3 mb-2">
            <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-sky-500 to-sky-600 flex items-center justify-center shadow-xl shadow-sky-500/20">
        ...
        <div className="flex gap-3">
          <Link href="/verify">
            <Button variant="outline" className="flex items-center gap-2 border-sky-200 hover:border-sky-400 hover:bg-sky-50">
          ...
          {user?.is_admin && (
            <Link href="/issue">
              <Button className="flex items-center gap-2 gradient-bg hover:opacity-90 shadow-lg shadow-sky-500/30">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 relative z-10">
        ...
        <Card className="premium-card border-l-4 border-l-sky-400 hover:scale-105 transition-transform duration-300">
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            ...
            <div className="w-12 h-12 rounded-xl bg-gradient-to-br from-sky-400 to-sky-500 flex items-center justify-center shadow-lg shadow-sky-400/30">
              <Activity className="h-6 w-6 text-white" />
            </div>
          </CardHeader>
          <CardContent>
            <div className="text-4xl font-bold" style={{color: '#0ea5e9'}}>{stats.totalVerified}</div>
        ...
        <Card className="premium-card border-l-4 border-l-sky-300 hover:scale-105 transition-transform duration-300">
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-semibold text-slate-600">{user?.is_admin ? "Pending" : "Available"}</CardTitle>
            <div className="w-12 h-12 rounded-xl bg-gradient-to-br from-sky-300 to-sky-400 flex items-center justify-center shadow-lg shadow-sky-300/30">
              <Users className="h-6 w-6 text-white" />
            </div>
          </CardHeader>
          <CardContent>
            <div className="text-4xl font-bold" style={{color: '#38bdf8'}}>{user?.is_admin ? stats.pending : stats.totalIssued}</div>
      ...
      <Card className="premium-card relative z-10">
        <CardHeader className="flex flex-row items-center justify-between">
          ...
          <Link href="/certificates">
            <Button variant="outline" size="sm" className="border-sky-200 hover:border-sky-400 hover:bg-sky-50">View All</Button>
          </Link>
        </CardHeader>
        <CardContent>
          {recentCerts.length > 0 ? (
            <div className="space-y-3">
              {recentCerts.map((cert) => (
                <div key={cert.id} className="flex items-center justify-between p-4 border border-slate-200 rounded-xl hover:border-sky-300 hover:shadow-lg hover:shadow-sky-500/10 transition-all bg-gradient-to-r from-white via-slate-50 to-white hover:from-sky-50/50 hover:via-sky-100/50 hover:to-sky-50/50">
                  <div className="flex items-center gap-4">
                    <div className="w-14 h-14 rounded-xl bg-gradient-to-br from-sky-500 to-sky-600 flex items-center justify-center shadow-xl shadow-sky-500/30">
                  ...
                  <div className="text-right">
                    ...
                    <div className="flex items-center gap-2 mt-2">
                      {cert.revoked ? (
                        ...
                        <span className="text-xs bg-sky-100 text-sky-700 px-3 py-1.5 rounded-full font-semibold border border-sky-200 shadow-sm">Signed</span>
                      ) : (
